...
representation = ast.to_s.gsub("\n", " ").squeeze(" ").gsub('"', '\"')

File.open(path / "representation.sql","w") do |f|
  f.write(%Q{INSERT INTO fixtures (track_slug, exercise_slug, representation, status, comments_data) VALUES ("ruby", "#{exercise_slug}", "#{representation}", '#{analysis["status"]}', '#{analysis["comments"].to_json}')})
end
